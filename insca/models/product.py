    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            if record.is_vault_product and not record.is_old_revision:
                res_code = self.env['res.code'].search([('name', '=', record.vault_code)])
                if res_code:
                    # Cambio tipo de datos
                    if record.vault_code:
                        vals.update({'vault_code': record.vault_code})
                    ...
                    if not vals.get('vault_categ'):
                        vals.update({'vault_categ': record.categ_id.name})
                    if not vals.get('vault_color'):
                        vals.update({'vault_color': record.vault_color})
                    if not vals.get('vault_route'):
                        vals.update({'vault_route': record.vault_route})
                    if not vals.get('vault_edge_code'):
                        vals.update({'vault_edge_code': record.vault_edge_code})
                    if not vals.get('vault_categ_terminado') and vals['vault_code'] == 'A00':
                        vals.update({'vault_categ_terminado': record.categ_id.name})

                    # Escribe material code
                    if not vals.get('vault_material_code'):
                        vals.update({'vault_material_code': record.vault_material_code})
                    ...
                    # Código A10
                        ...
                        if len(record.bom_ids) == 0:
                            if vals.get('vault_material_code') or vals['vault_edge_code'] or vals['vault_color']:
                                ...
                                mrp_bom_object.sudo().create({'product_tmpl_id': record.id,
                                                              'code': record.vault_revision,
                                                              'product_qty': 1,
                                                              'type': 'normal',
                    ...
                    # Código A30
                        ...
                        if not len(product_zero):
                            raise ValidationError(_('Producto %s no encontrado. Revise Vault'
                                                    % (record.default_code[:-3] + '000')))

                        # Crear lista de materiales
                        if len(record.bom_ids) == 0 and record.default_code[-3:] == '000':
                        ...
                        if len(record.bom_ids) == 0 and record.default_code[-3:] != '000':
                            if vals['vault_color']:
                                ...
                                product_ids_max = self.env['product.product'].\
                                    search([('default_code', '=', record.default_code[:-3] + '000')])
                                if vals['vault_color']:
                                    product_ids += self.env['product.product'].search([('default_code', '=',
                                                                                        vals['vault_color'])])
